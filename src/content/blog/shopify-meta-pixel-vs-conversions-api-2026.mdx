...
- [iOS 17+ continues to expand tracking restrictions](/blog/ios-attribution-gap-shopify-tracking-fix). Browser pixel signal coverage has dropped to ~60-70% of actual events for most DTC stores.
...
**Running both browser pixel and CAPI without deduplication.** This [double-counts every event](/blog/google-ads-conversion-tracking-double-counting-fix). Your ROAS in Ads Manager looks great. Your actual revenue doesn't move. We've fixed this in audits and watched reported ROAS drop 40% — to its actual level.
...
The [reported ROAS lift is partly artificial](/blog/meta-ads-retargeting-roas-overstated-fix) — you're capturing events that always existed but weren't visible before. Your actual revenue won't jump. But Meta now has the signal to optimize correctly, which is where the real performance gain comes from over the following month.
