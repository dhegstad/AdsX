...
The replacement is **Shopify Functions**: logic compiled to WebAssembly that runs in roughly **sub-5ms with no cold starts**. The Scripts sunset is one of the load-bearing changes in [Shopify's 2026 Editions](/blog/shopify-editions-2026-new-features), not a minor deprecation. Functions are faster and more capable than Scripts, but they are not a toggle. Most merchants migrate either by installing a public app that delivers the same behavior or by deploying a custom Function. Either way, it's work, and the clock is the problem.
