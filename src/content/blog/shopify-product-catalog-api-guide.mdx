...
If you only take one thing away: **use the GraphQL Admin API, not REST.** REST is legacy as of 2024–2025, and every new catalog capability ships to GraphQL first. This guide assumes the GraphQL Admin API throughout, so you'll need a [Shopify store](https://shopify.pxf.io/c/6318547/3797171/13624) with Admin API access to run the examples. For authentication, OAuth, and rate-limit fundamentals, see the [Shopify Admin API guide](/blog/shopify-admin-api-guide); for the read-side browsing API, see the [Shopify Storefront API guide](/blog/shopify-storefront-api-guide).
