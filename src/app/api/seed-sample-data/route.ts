...
import { adAccount, changeEvent, organization } from "@/lib/db/schema";
...
export async function GET() {
  try {
    ...
    // If no accounts, create a test one
    ...
    for (const account of accounts) {
      for (const event of sampleEvents) {
        const resourceId = `${event.entityType}_${Math.random().toString(36).substr(2, 9)}`;
        ...
        // Create events at different times in the past (0-48 hours ago)
        const detectedAt = new Date();
        detectedAt.setHours(detectedAt.getHours() - Math.floor(Math.random() * 48));

        // Determine severity based on change type
        const severity =
          event.changeType === "deleted" || event.changeType.includes("decreased")
            ? "warning"
            : event.changeType === "paused"
            ? "critical"
            : "info";
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully seeded ${eventsCreated} sample change events!`,
      accounts: accounts.length,
      eventsPerAccount: sampleEvents.length,
    });
  } catch (error) {
