    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
      ...
      <div className="border-b border-white/10 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm text-white/50">Last 7 days</div>
        </div>
      </div>

      {/* Stats Row */}
      <div className="grid grid-cols-2 lg:grid-cols-5 border-b border-white/10">
        {data.map((item) => {
          const platform = platforms.find((p) => p.name === item.platform);
          return (
            <div
              key={item.platform}
              className="px-6 py-4 border-r border-white/10 last:border-r-0"
            >
      ...
      <div className="border-t border-white/10 px-6 py-4 bg-white/[0.02]">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-white/50">AI Visibility Score</div>
            <div className="text-3xl font-bold text-emerald-400">{visibilityScore}%</div>
          </div>
          <div className="text-right">
            <div className="text-sm text-white/50">Total Mentions</div>
